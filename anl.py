...
DayBeforeYesterday=Yesterday-OneDay
DayBeforeYesterdayT=DayBeforeYesterday.timetuple()
DayBeforeYesterdayDOY=DayBeforeYesterdayT.tm_yday
...
#(ANLTimeY,ANLIrradianceY) = GetIrradiaceANLobservations(DayBeforeYesterdayDOY)
...
if(len(np.array(ANLTimeY).ravel()) != 4*24):
    print 'Error observational dataset is incomplete'
    plt.figtext(.35, .5, 'Error: Incomplete persistence inputs!\nCliPer forecast will resume shortly.', fontsize=15)
...
ANLForecastIrr=np.minimum(np.maximum(ANLClimIrr[LenANLObs-1:LenANLObs-1+ForecastHours*4]-(ANLClimIrr[LenANLObs-1]-ANLObsIrr[LenANLObs-1]),ANLClimIrrMin[LenANLObs-1:LenANLObs-1+ForecastHours*4]),ANLClimIrrMax[LenANLObs-1:LenANLObs-1+ForecastHours*4])
...
print 'CliPer forecast:'
lT=list((ANLForecastTime-arrdate[0][0])*24)
lF=list(ANLForecastIrr)
for i in range(len(lT)):
    print str(int(lT[i])).zfill(2)+':'+str(int((lT[i]-int(lT[i]))*60.)).zfill(2)+' - '+str(lF[i])
...
plt.gca().set_position((.125, .18, .8, .78))
#\nArgonne National Laboratory - emconsta@mcs.anl.gov\n 
plt.figtext(.16, .04, 'Argonne National Laboratory - emconsta@mcs.anl.gov', fontsize=15)
...
#Recent modifications made by Edwin Campos (ecampos@anl.gov) and Emil Constantinescu
